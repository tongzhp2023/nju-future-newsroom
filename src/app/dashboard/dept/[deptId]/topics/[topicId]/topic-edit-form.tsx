...
import { updateTopicDraft, submitTopicWithUpdate, resubmitTopic, revertTopicToDraft } from '@/lib/actions'
import type { TopicWithDetails, TopicFormField } from '@/lib/types'

/**
 * 选题编辑表单 — 草稿和已驳回两种模式
 * 与新建选题表单(topic-form.tsx)保持完全一致的 UI 风格
 */
export default function TopicEditForm({
  ...
  const [formData, setFormData] = useState<Record<string, string>>(() => {
    ...
    formTemplate.forEach((field) => {
      initial[field.key] = topic.form_data?.[field.key]
        ? String(topic.form_data[field.key])
        : ''
    })
    return initial
  })
  ...
  // 记录初始值用于判断是否有修改（已驳回重新提交时需要）
  const [initialTitle] = useState(topic.title)
  ...
  const hasChanges = () => {
    ...
    return formTemplate.some(field => formData[field.key] !== initialFormData[field.key])
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'draft') {
        await submitTopicWithUpdate(topic.id, title.trim(), formData)
      } else {
        // 已驳回模式：重新提交
        if (!hasChanges()) {
          setError('请先修改选题内容后再重新提交')
          setSubmitting(false)
          return
        }
        await resubmitTopic(topic.id, title.trim(), formData)
      }
      router.push(`/dashboard/dept/${deptId}/topics?status=pending`)
    } catch (err) {
  ...
  // 草稿模式：保存草稿 / 已驳回模式：重新保存为草稿
  const handleSaveDraft = async () => {
    setError(null)
    setSavingDraft(true)
    try {
      if (mode === 'draft') {
        await updateTopicDraft(topic.id, title.trim(), formData)
        router.push(`/dashboard/dept/${deptId}/topics?status=draft`)
      } else {
        // 已驳回 → 保存为草稿
        await revertTopicToDraft(topic.id, title.trim(), formData)
        router.push(`/dashboard/dept/${deptId}/topics?status=draft`)
      }
    } catch (err) {
